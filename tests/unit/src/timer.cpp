...
const tst::set set("timer", [](tst::suite& suite) {
	suite.add("basic", []() {
		...
		auto timer = utki::make_shared<ruis::timer>(updater, [&](uint32_t elapsed_ms) {
			++num_triggered;
	...
	suite.add("restart_from_callback", []() {
		...
		utki::shared_ref<ruis::timer> timer = utki::make_shared<ruis::timer>(updater, [&](uint32_t elapsed_ms) {
			++num_triggered;
			timer.get().start(10);
		});
