...
morda::Vec2r LinearContainer::ComputeMinDim()const NOEXCEPT{
	...
	std::list<std::shared_ptr<Widget>> keepRatio;
	
	for(auto& p : this->Children()){
		auto& lp = this->GetLayoutParamsAs<LayoutParams>(*p);
		
		if(lp.keepRatio){
			keepRatio.push_back(p);
		}else{
			morda::Vec2r dim = lp.DimForWidget(*p);
	...
	for(auto i = keepRatio.begin(); i != keepRatio.end();){
		...
		ASSERT(dim[transIndex] >= 0)
		
		if(dim[transIndex] != 0){
			ASSERT((*i)->GetMinDim()[transIndex] >= 0)
			if((*i)->GetMinDim()[transIndex] == 0){
				dim[transIndex] = 0;
				i = keepRatio.erase(i);
			}else{
				float ratio = (*i)->GetMinDim()[longIndex] / (*i)->GetMinDim()[transIndex];

				float offerRatio = dim[longIndex] / dim[transIndex];
