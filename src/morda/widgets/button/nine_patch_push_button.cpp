		...
		nine_patch_button(this->context, desc)
{
	if(!this->get_unpressed_nine_patch()){
		this->set_unpressed_nine_patch(this->context->loader.load<res_ninepatch>("morda_npt_button_normal"));
	}
	
	if(!this->get_pressed_nine_patch()){
		this->set_pressed_nine_patch(this->context->loader.load<res_ninepatch>("morda_npt_button_pressed"));
	}
