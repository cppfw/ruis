...
nine_patch::nine_patch( //
	...
	padding(
		...
		{
			.params{
				.container = std::move(params.params.container),
				.specific = std::move(params.params.padding_params)
			}
		},
		std::move(children)
	),
...
void nine_patch::render_nine_patch(const mat4& matrix) const
{
	...
	// left-top
	{
		...
		matr.scale(this->get_container().rect().p);
	...
	// top
	{
		ruis::mat4 matr(matrix);
		matr.translate(
			this->get_container().rect().p.x(), //
			0
		);
		matr.scale(
			this->get_container().rect().d.x(), //
			this->get_container().rect().p.y()
		);
	...
	// right-top
	{
		ruis::mat4 matr(matrix);
		matr.translate(
			this->get_container().rect().x2(), //
			0
		);
		matr.scale(
			this->rect().d.x() - this->get_container().rect().x2(), //
			this->get_container().rect().p.y()
		);
	...
	{
		ruis::mat4 matr(matrix);
		matr.translate(this->get_container().rect().p);
		matr.scale(this->get_container().rect().d);
	...
	// left-bottom
	{
		...
		matr.translate(
			0, //
			this->get_container().rect().y2()
		);
		matr.scale(
			this->get_container().rect().p.x(), //
			this->rect().d.y() - this->get_container().rect().y2()
		);
	...
	// right-bottom
	{
		...
		auto content_x2_y2 = this->get_container().rect().x2_y2();
