...
void tiling_area::on_lay_out()
{
	...
	for (const auto& t : this->get_container()) {
		tiles_length += max( //
	...
	this->get_container().resize(content_dims);
	...
	auto num_draggers = [&]() -> size_t {
		if (this->get_container().empty()) {
			...
			return this->get_container().size() - 1;
		}
	}();
	...
	for (auto i = std::next(this->begin()); i != this->end(); ++i) {
		...
		ASSERT(index < this->get_container().size())

		auto& dragger = dynamic_cast<internal::dragger&>(i->get());

		dragger.prev_widget = this->get_container().children()[index].to_shared_ptr();
		dragger.next_widget = this->get_container().children()[index + 1].to_shared_ptr();
