	...
	return ruis::make::gap(c,
		{
			.layout_params{
				.dims{
					ruis::styled<ruis::layout::dimension>(ruis::layout::dimension(c.get().style().get_len_indent().get())),
...
void tree_view::provider_base::set_iter_to(size_t index) const
{
	if (index != this->iter_index) {
		if (index > this->iter_index) {
			this->iter = utki::next(
				this->iter, //
				index - this->iter_index
			);
		} else {
			utki::assert(index < this->iter_index, SL);
			this->iter = utki::prev(
				this->iter, //
...
void tree_view::provider_base::remove_children(decltype(iter) from)
{
...
void tree_view::provider_base::notify_item_added(utki::span<const size_t> index)
{
	...
	utki::assert(p->value.subtree_size == 0, SL);
...
void tree_view::provider_base::notify_item_removed(utki::span<const size_t> index)
{
	...
	if (utki::deep_greater_or_equals(utki::make_span(cur_iter_index), index)) {
		...
		if (cur_iter_index < next_index) {
			while (this->iter != ri) {
				utki::assert(this->iter_index != 0, SL);
				--this->iter;
				--this->iter_index;
			}
		} else {
